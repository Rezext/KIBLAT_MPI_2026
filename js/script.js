...
// ========================================
// UPGRADE FITUR ABSENSI PUBLIC (RINGKAS)
// ========================================

// Render Absensi Public (Ringkas dengan Preview 5 Orang)
async function renderAbsensiPublic() {
    const container = document.getElementById('absensiPublicList');
    if (!container) return;
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            attendanceSnapshot.forEach(attDoc => {
                hadirList.push(attDoc.data());
            });
            
            const totalHadir = hadirList.length;
            const totalSemua = Object.keys(MEMBERS_DATA.members).length;
            const persentase = Math.round((totalHadir / totalSemua) * 100);
            
            // Tampilkan 5 orang pertama
            const preview = hadirList.slice(0, 5);
            const hasMore = hadirList.length > 5;
            
            html += `
                <div class="absensi-card">
                    <div class="absensi-header">
                        <h3>📋 ${data.acara}</h3>
                        <span class="absensi-date">📅 ${data.tanggal} | ⏰ ${data.startTime} - ${data.endTime} WITA</span>
                    </div>
                    ...
                    <div class="absensi-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-number">${totalSemua - totalHadir}</span>
                            <span class="stat-label">Tidak Hadir</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number">${persentase}%</span>
                            <span class="stat-label">Kehadiran</span>
                        </div>
                    </div>
                    
                    <div class="absensi-preview">
                        <h4>✅ Daftar Hadir (${totalHadir} orang):</h4>
                        <div class="preview-list">
                            ${preview.map((h, i) => `
                                <div class="preview-item">
                                    <span class="preview-no">${i + 1}.</span>
                                    <span class="preview-name">${h.nama}</span>
                                    <span class="preview-time">${h.waktu}</span>
                                </div>
                            `).join('')}
                            ${hasMore ? `<div class="preview-more">... dan ${hadirList.length - 5} orang lainnya</div>` : ''}
                        </div>
                    </div>
                    
                    <div class="absensi-actions">
                        <button onclick="viewAbsensiDetailPublic('${sessionId}')" class="btn-detail">
                            📄 Lihat Selengkapnya
                        </button>
                        <button onclick="downloadAbsensiExcel('${sessionId}', '${data.acara}')" class="btn-download">
                            📥 Download Excel
                        </button>
                    </div>
                </div>
        ...
        console.error('Error loading public absensi:', error);
        container.innerHTML = '<p class="error-state">❌ Gagal memuat data absensi</p>';
        hideLoading();
    }
}

// View Detail Absensi Public (Modal Lengkap)
async function viewAbsensiDetailPublic(sessionId) {
    showLoading();
    ...
    try {
        ...
        attendanceSnapshot.forEach(doc => {
            const data = doc.data();
            hadirList.push(data);
            hadirNIMs.add(String(data.nim).trim());
        });
        
        // Filter tidak hadir
        const tidakHadirList = [];
        Object.keys(MEMBERS_DATA.members).forEach(nim => {
            if (!hadirNIMs.has(String(nim).trim())) {
                tidakHadirList.push({
                    nim: nim,
                    nama: MEMBERS_DATA.members[nim].nama
                });
        ...
        showAbsensiDetailModal(sessionData, hadirList, tidakHadirList);
...
// Modal Detail Absensi (Full List)
function showAbsensiDetailModal(sessionData, hadirList, tidakHadirList) {
    const modal = document.createElement('div');
    modal.className = 'modal';
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 800px;">
            ...
            <h2 style="text-align: center; margin-bottom: 10px;">📋 Detail Absensi</h2>
            <h3 style="text-align: center; color: #667eea; margin-bottom: 20px;">${sessionData.acara}</h3>
            <p style="text-align: center; color: #7f8c8d; margin-bottom: 30px;">
                📅 ${sessionData.tanggal} | ⏰ ${sessionData.startTime} - ${sessionData.endTime} WITA
            </p>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
                <div style="background: #d5f4e6; padding: 20px; border-radius: 10px; text-align: center;">
                    <div style="font-size: 36px; font-weight: 700; color: #27ae60;">✅ ${hadirList.length}</div>
                    <div style="color: #27ae60; font-weight: 600;">Hadir</div>
                </div>
                <div style="background: #fadbd8; padding: 20px; border-radius: 10px; text-align: center;">
                    <div style="font-size: 36px; font-weight: 700; color: #e74c3c;">❌ ${tidakHadirList.length}</div>
                    <div style="color: #e74c3c; font-weight: 600;">Tidak Hadir</div>
                </div>
            </div>
            
            <div style="margin-bottom: 30px;">
                <h3 style="color: #27ae60; margin-bottom: 15px;">✅ Daftar Hadir (${hadirList.length})</h3>
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead style="background: #667eea; color: white; position: sticky; top: 0;">
                            <tr>
                                ...
                                <th style="padding: 12px; text-align: left;">NIM</th>
                                <th style="padding: 12px; text-align: left;">Nama</th>
                                <th style="padding: 12px; text-align: center;">Waktu</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${hadirList.map((h, i) => `
                                <tr style="border-bottom: 1px solid #f0f0f0;">
                                    ...
                                    <td style="padding: 10px;">${h.nim}</td>
                                    <td style="padding: 10px;">${h.nama}</td>
                                    <td style="padding: 10px; text-align: center;">${h.waktu}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
            ...
            <div style="margin-bottom: 20px;">
                <h3 style="color: #e74c3c; margin-bottom: 15px;">❌ Tidak Hadir (${tidakHadirList.length})</h3>
                <div style="max-height: 300px; overflow-y: auto; border: 1px solid #ddd; border-radius: 8px;">
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody>
                            ${tidakHadirList.map((t, i) => `
                                <tr style="border-bottom: 1px solid #f0f0f0;">
                                    <td style="padding: 10px;">${i + 1}</td>
                                    <td style="padding: 10px;">${t.nim}</td>
            ...
            <div style="text-align: center;">
                <button onclick="this.closest('.modal').remove()" style="padding: 12px 30px; background: #95a5a6; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">
                    Tutup
                </button>
            </div>
        </div>
...
// Download Absensi ke Excel (Format: NO | NAMA | NIM | WAKTU)
async function downloadAbsensiExcel(sessionId, acaraNama) {
    showLoading();
    ...
    try {
        ...
        const excelData = [
            ['LAPORAN KEHADIRAN'],
            ['Acara:', sessionData.acara],
            ['Tanggal:', sessionData.tanggal],
            ['Waktu:', `${sessionData.startTime} - ${sessionData.endTime} WITA`],
            ['Total Hadir:', hadirList.length],
            [],
            ['NO', 'NAMA', 'NIM', 'WAKTU']
        ];
        ...
        // Generate filename
        const filename = `Absensi_${acaraNama.replace(/\s+/g, '_')}_${sessionData.tanggal.replace(/\s+/g, '_')}.xlsx`;
